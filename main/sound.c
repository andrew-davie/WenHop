...
const unsigned char sampleDiamond2[] = {      // C,F,V,LEN
   5,14,1,   1,
   5,12,2,   1,
   5,11,3,   1,
   5,3,4,   2,
...
const unsigned char sampleDiamond3[] = {      // C,F,V,LEN
   5,14,1,   1,
   5,11,3,   1,
   5,3,4,   1,
   5,4,3,   1,
...
void playAudio() {
    ...
    for (unsigned int id = 0; id < SFX_MAX; id++) {
        ...
        if (audioRequest[id]) {
            ...
            if (lowest >= 0 && (!Track[lowest].id
                || AudioSamples[id].priority >= AudioSamples[Track[lowest].id].priority)) {
...
void processTracks() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        RAM[_BUF_AUDV + i] = volume[i] = (instrument[track[i].instrument][envelope_ptr]
            * sound_volume * track[i].volume) >> (4 + 10 + 8);
